...
- **Wordmark do hero**: o H1 da landing usa `public/images/wordmark_sitio_da_voinha.svg` (extraído do logotipo: estampa "SÍTIO DA" + assinatura "Voinha"; `viewBox="0 0 2100 1864"`; paths `texto-sitio-da` + `escrita-voinha-e-talheres`). Renderizado **sempre branco** com a classe `.logo-sdv-hero` (`filter: brightness(0) invert(1) drop-shadow(...)`) — não depende do dark mode porque está sobre a foto do hero. Não é "fonte" editável: se o lettering mudar, re-extrair do logotipo.
