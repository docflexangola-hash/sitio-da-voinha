...
- **Fix da renderização da Galeria (`ReferenceError: active is not defined`)**: a secção `#galeria` permanecia oculta (nem título aparecia) mesmo com 3 fotos na BD e REST 200. Causa-raiz: em `src/main.js` (`renderList`, linha de acessibilidade `aria-labelledby`) usava-se `active.catId` em vez de `cat.catId` — a variável local é `const cat = activeCat()`; `active` só existe noutras funções. O `Uncaught ReferenceError` rebentava **dentro** de `renderMenu()`, que chama `renderTabs(); renderList(); renderGaleria();` — `renderGaleria()` nunca corria e a secção ficava `hidden`. O menu continuava a pintar (o `innerHTML` é escrito antes da linha a errar) — daí os chips responderem no teste do dono. Corrigido para `cat.catId`; build verde; **confirmado em produção pelo dono** (push `5e3e430` → deploy Vercel). Lição: um ReferenceError neste pendão de render mata a galeria em silêncio e `npm run build` não o apanha (esbuild não faz typecheck) — quando "tudo está certo mas não aparece", diagnosticar no browser/DevTools, não no servidor.
