...
- **Fix da renderização da Galeria (`ReferenceError: active is not defined`)**: a secção `#galeria` permanecia oculta porque em `src/main.js` (`renderList`) a linha de acessibilidade `aria-labelledby` usava `active.catId` em vez de `cat.catId`. O erro fatal interrompia a execução de `renderMenu()` antes de `renderGaleria()` ser invocado. Corrigido para `cat.catId` e build validado (`npm run build`).
